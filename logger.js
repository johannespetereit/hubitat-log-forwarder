...
const hubitatWeb = require('./hubitatweb')
...
(async () => {
    ...
    const baseUrl = "https://" + config.hub.hub_host;
    const session = await hubitatWeb.login(baseUrl)
    var apps = await hubitatWeb.getApps(baseUrl);
    ...
    if (webserver.enable) {
        ...
        server.get('/get_devices', function (req, res) {
            ...
            request(webserver.maker_api_base_url + process.env.HUBITAT_MAKER_API_TOKEN, { json: true }, (err, resp, body) => {
